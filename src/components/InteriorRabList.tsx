...
interface Props {
  ...
  onDeleteDraft?: (projectId: string, revisionId: string) => void;
  onDeleteProject?: (projectId: string) => void;
}

export const InteriorRabList: React.FC<Props> = ({ projects, onOpenDraft, onCreateProject, onDeleteDraft, onDeleteProject }) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [draftToDelete, setDraftToDelete] = useState<{ project: InteriorProject; revision: InteriorRABRevision } | null>(null);
  const [projectToDelete, setProjectToDelete] = useState<InteriorProject | null>(null);
                          ...
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            {onDeleteProject && (
                              <button
                                ...
                                title="Hapus Proyek & Semua Draft"
                              >
                                      ...
                                      <div className="flex items-center gap-4 self-end sm:self-center">
                                        ...
                                        <div className="flex items-center gap-1.5">
                                          ...
                                          {onDeleteDraft && (
                                            <button
                                              type="button"
                                              onClick={() => setDraftToDelete({ project, revision: rev })}
                                              className="px-2.5 py-1.5 rounded text-xs font-semibold flex items-center gap-1 transition cursor-pointer text-rose-600 hover:text-white bg-rose-50 hover:bg-rose-600 border border-rose-200 hover:border-rose-600 shadow-2xs"
                                              title="Hapus Draft Revisi"
                                            >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden animate-scale-up">
            <div className="p-5 border-b border-slate-100 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">Hapus Draft RAB</h3>
                <p className="text-xs text-slate-500">Tindakan ini tidak dapat dibatalkan</p>
              </div>
            </div>
            <div className="p-5 text-xs text-slate-600 space-y-3">
              <p>
                Apakah Anda yakin ingin menghapus draft <span className="font-bold text-slate-900">{draftToDelete.revision.name}</span> (Rev.0{draftToDelete.revision.version}) untuk proyek <span className="font-semibold text-slate-800">{draftToDelete.project.name}</span>?
              </p>
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-[11px] flex items-start gap-2">
                ...
                <span>RAB draft ini beserta rincian item pekerjaan di dalamnya akan dihapus.</span>
              </div>
            </div>
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-2">
              ...
              <button
                ...
                onClick={() => {
                  if (onDeleteDraft) {
                    onDeleteDraft(draftToDelete.project.id, draftToDelete.revision.id);
                  }
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden animate-scale-up">
            <div className="p-5 border-b border-slate-100 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">Hapus Proyek &amp; Seluruh Draft</h3>
                <p className="text-xs text-slate-500">Tindakan ini tidak dapat dibatalkan</p>
              </div>
            </div>
            <div className="p-5 text-xs text-slate-600 space-y-3">
