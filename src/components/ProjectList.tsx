...
import { exportProjectListToExcel } from "../utils/excelExport";
import { exportProjectListToPdf } from "../utils/pdfExport";
...
export const ProjectList: React.FC<ProjectListProps> = ({
  ...
  const countByStatus = {
    "On Tender": projects.filter((p) => p.status === "On Tender").length,
  ...
  return (
    <div className="flex-1 h-full min-h-0 w-full overflow-y-auto bg-slate-100 font-sans p-4 sm:p-6 space-y-5 pb-24">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base sm:text-lg font-bold text-slate-900 leading-tight">
                  Project List (Master Manajemen Proyek)
                </h2>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <ExportDropdown
              ...
              onExportExcel={() => exportProjectListToExcel(projects)}
              onExportPdf={() => exportProjectListToPdf(projects)}
              menuTitle="Project Portfolio Export"
            />
        ...
        {lastUpdatedNotice && (
          <div className="mt-3 px-3.5 py-2 bg-emerald-50 border border-emerald-300 text-emerald-800 rounded-lg text-xs flex items-center justify-between animate-fade-in">
            <div className="flex items-center gap-2">
              <Check className="w-4 h-4 text-emerald-600 shrink-0" />
              <span>
                <strong>SQLite Real-time Update:</strong> Status proyek{" "}
                <code className="bg-emerald-100 px-1 py-0.5 rounded font-mono">
                  {projects.find((p) => p.id === lastUpdatedNotice.id)?.projectCode}
                </code>{" "}
                berhasil diperbarui ke{" "}
          ...
          <table className="w-full text-left text-xs border-collapse min-w-[1050px]">
            <thead>
              <tr className="bg-slate-50 text-slate-600 border-b border-slate-200 font-bold uppercase tracking-wider text-[11px]">
                <th className="py-3 px-4 w-40 ">Project Code</th>
                <th className="py-3 px-4 min-w-[240px]">Project Name &amp; Description</th>
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 align-top break-words whitespace-normal text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            type="button"
                            onClick={() => onOpenScheduleDraft(project)}
                            className="px-2.5 py-1.5 bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white border border-indigo-200 hover:border-indigo-600 rounded-md text-[11px] font-semibold flex items-center gap-1 transition cursor-pointer"
                            title="Buka Schedule & Kurva S"
                          >
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div>
                  ...
                  <div className="font-mono text-indigo-700 font-bold">{infoProject.projectCode}</div>
                </div>
                <div>
                  <div className="text-slate-500 font-bold mb-1">Status Siklus</div>
                  <div className="font-semibold text-slate-800">{infoProject.status}</div>
                </div>
                <div className="col-span-2">
                  <div className="text-slate-500 font-bold mb-1">Nama Proyek</div>
                  <div className="font-semibold text-slate-900 text-sm">{infoProject.name}</div>
                </div>
                <div className="col-span-2">
                  <div className="text-slate-500 font-bold mb-1">Klien / Pemilik</div>
                  <div className="text-slate-800">{infoProject.ownerName || "-"}</div>
                </div>
                <div className="col-span-2">
                  <div className="text-slate-500 font-bold mb-1">Lokasi</div>
                  <div className="text-slate-800">{infoProject.location || "-"}</div>
                </div>
                <div>
                  <div className="text-slate-500 font-bold mb-1">Luas Bangunan</div>
                  <div className="text-slate-800">{infoProject.requirements?.buildingArea || 0} m²</div>
                </div>
                <div>
                  <div className="text-slate-500 font-bold mb-1">Jumlah Lantai</div>
                  <div className="text-slate-800">{infoProject.requirements?.floorsCount || 1}</div>
                </div>
                <div className="col-span-2">
                  <div className="text-slate-500 font-bold mb-1">Deskripsi</div>
                  <div className="text-slate-700 bg-slate-50 p-2 rounded border border-slate-100">{infoProject.description || "Tidak ada deskripsi"}</div>
                </div>
              </div>
              <div className="pt-4 flex justify-end border-t border-slate-100">
                <button onClick={() => setInfoProject(null)} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded text-xs font-semibold cursor-pointer">Tutup</button>
              </div>
            </div>
          </div>
        </div>
            ...
            >
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditProject({ ...editProject, projectCode: e.target.value })}
                    className="w-full px-3 py-1.5 bg-slate-50 border border-slate-300 rounded text-xs font-mono font-bold text-indigo-700"
                    required
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="text"
                    value={editProject.ownerName}
                    onChange={(e) => setEditProject({ ...editProject, ownerName: e.target.value })}
                    className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded text-xs text-slate-800"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="number"
                    value={editProject.requirements?.buildingArea || 0}
                    onChange={(e) => setEditProject({
                      ...editProject,
                      requirements: { ...editProject.requirements, buildingArea: Number(e.target.value) || 0 }
                    })}
                    className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded text-xs text-slate-800"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={editProject.requirements?.floorsCount || 1}
                    onChange={(e) => setEditProject({
                      ...editProject,
                      requirements: { ...editProject.requirements, floorsCount: Number(e.target.value) || 1 }
                    })}
                    className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded text-xs text-slate-800"
                  />
                </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden animate-scale-up">
            ...
            <div className="p-5 text-xs text-slate-600 space-y-3">
              ...
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-[11px] flex items-start gap-2">
                ...
                <span>Seluruh data RAB, schedule, dan dokumen terkait proyek ini di Project Workspace akan dihapus permanen.</span>
              </div>
            </div>
